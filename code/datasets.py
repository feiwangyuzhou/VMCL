from torch.utils.data import Dataset, DataLoader
import lmdb
from utils import deserialize
import numpy as np
import torch
import pdb


class TrainSubgraphDataset(Dataset):
    def __init__(self, args):
        self.args = args
        self.env = lmdb.open(args.db_path, readonly=True, max_dbs=5, lock=False)
        self.subgraphs_db = self.env.open_db("train_subgraphs".encode())

    def __len__(self):
        return self.args.num_train_subgraph

    @staticmethod
    def collate_fn(data):
        return data

    def __getitem__(self, idx):
        with self.env.begin(db=self.subgraphs_db) as txn:
            str_id = '{:08}'.format(idx).encode('ascii')
            sup_tri, sup_tri_self, que_tri, hr2t, rt2h, ht2r, nrel = deserialize(txn.get(str_id))

        nentity = len(np.unique(np.array(sup_tri)[:, [0, 2]]))

        # pdb.set_trace()
        sup_neg_tail_ent = [np.random.choice(np.delete(np.arange(nentity), hr2t[(h, r)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]

        sup_neg_head_ent = [np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]
        # pdb.set_trace()
        sup_neg_rel = [np.random.choice(np.delete(np.arange(nrel), ht2r[(h, t)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]

        que_neg_tail_ent = [np.random.choice(np.delete(np.arange(nentity), hr2t[(h, r)]),
                                        self.args.metatrain_num_neg) for h, r, t in que_tri]

        que_neg_head_ent = [np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t)]),
                                        self.args.metatrain_num_neg) for h, r, t in que_tri]

        que_neg_rel = [np.random.choice(np.delete(np.arange(nrel), ht2r[(h, t)]),
                                        self.args.metatrain_num_neg) for h, r, t in que_tri]

        sup_tri = torch.from_numpy(np.array(sup_tri))
        sup_neg_tail_ent = torch.from_numpy(np.array(sup_neg_tail_ent))
        sup_neg_head_ent = torch.from_numpy(np.array(sup_neg_head_ent))
        sup_neg_rel = torch.from_numpy(np.array(sup_neg_rel))
        sup_tri_self = torch.from_numpy(np.array(sup_tri_self))
        que_tri = torch.from_numpy(np.array(que_tri))
        que_neg_tail_ent = torch.from_numpy(np.array(que_neg_tail_ent))
        que_neg_head_ent = torch.from_numpy(np.array(que_neg_head_ent))
        que_neg_rel = torch.from_numpy(np.array(que_neg_rel))

        return sup_tri, sup_neg_tail_ent, sup_neg_head_ent, sup_neg_rel, sup_tri_self, \
               que_tri, que_neg_tail_ent, que_neg_head_ent, que_neg_rel

class ValidSubgraphDataset(Dataset):
    def __init__(self, args):
        self.args = args
        self.env = lmdb.open(args.db_path, readonly=True, max_dbs=5, lock=False)
        self.subgraphs_db = self.env.open_db("valid_subgraphs".encode())

    def __len__(self):
        txn = self.env.begin(db=self.subgraphs_db)
        num = txn.stat()['entries']
        return num

    @staticmethod
    def collate_fn(data):
        return data

    def __getitem__(self, idx):
        with self.env.begin(db=self.subgraphs_db) as txn:
            str_id = '{:08}'.format(idx).encode('ascii')
            sup_tri, sup_tri_self, que_tri, hr2t, rt2h, ht2r, nrel = deserialize(txn.get(str_id))

        nentity = len(np.unique(np.array(sup_tri)[:, [0, 2]]))

        que_dataset = KGEEvalDataset(self.args, que_tri, nentity, hr2t, rt2h, ht2r)
        que_dataloader = DataLoader(que_dataset, batch_size=len(que_tri),
                                    collate_fn=KGEEvalDataset.collate_fn)

        sup_neg_tail_ent = [np.random.choice(np.delete(np.arange(nentity), hr2t[(h, r)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]

        sup_neg_head_ent = [np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]
        # pdb.set_trace()
        sup_neg_rel = [np.random.choice(np.delete(np.arange(nrel), ht2r[(h, t)]),
                                        self.args.metatrain_num_neg) for h, r, t in sup_tri]

        sup_tri = torch.from_numpy(np.array(sup_tri))
        sup_neg_tail_ent = torch.from_numpy(np.array(sup_neg_tail_ent))
        sup_neg_head_ent = torch.from_numpy(np.array(sup_neg_head_ent))
        sup_neg_rel = torch.from_numpy(np.array(sup_neg_rel))
        sup_tri_self = torch.from_numpy(np.array(sup_tri_self))

        return sup_tri, que_dataloader, sup_tri, sup_neg_tail_ent, sup_neg_head_ent, sup_neg_rel, sup_tri_self


class KGETrainDataset(Dataset):
    def __init__(self, args, train_triples, num_ent, num_rel, num_neg, hr2t, rt2h, ht2r):
        self.args = args
        self.triples = train_triples
        self.num_ent = num_ent
        self.num_rel = num_rel
        self.num_neg = num_neg
        self.hr2t = hr2t
        self.rt2h = rt2h
        self.ht2r = ht2r

    def __len__(self):
        return len(self.triples)

    def __getitem__(self, idx):
        pos_triple = self.triples[idx]
        h, r, t = pos_triple

        neg_tail_ent = np.random.choice(np.delete(np.arange(self.num_ent), self.hr2t[(h, r)]),
                                        self.num_neg)

        neg_head_ent = np.random.choice(np.delete(np.arange(self.num_ent), self.rt2h[(r, t)]),
                                        self.num_neg)

        neg_head_rel = np.random.choice(np.delete(np.arange(self.num_rel), self.ht2r[(h, t)]),
                                        self.num_neg)

        pos_triple = torch.LongTensor(pos_triple)
        neg_tail_ent = torch.from_numpy(neg_tail_ent)
        neg_head_ent = torch.from_numpy(neg_head_ent)
        neg_head_rel = torch.from_numpy(neg_head_rel)

        return pos_triple, neg_tail_ent, neg_head_ent, neg_head_rel

    @staticmethod
    def collate_fn(data):
        pos_triple = torch.stack([_[0] for _ in data], dim=0)
        neg_tail_ent = torch.stack([_[1] for _ in data], dim=0)
        neg_head_ent = torch.stack([_[2] for _ in data], dim=0)
        neg_rel = torch.stack([_[3] for _ in data], dim=0)
        return pos_triple, neg_tail_ent, neg_head_ent, neg_rel


class KGEEvalDataset(Dataset):
    def __init__(self, args, eval_triples, num_ent, hr2t, rt2h, ht2r):
        self.args = args
        self.triples = eval_triples
        self.num_ent = num_ent
        self.hr2t = hr2t
        self.rt2h = rt2h
        self.ht2r = ht2r
        self.num_cand = 'all'

    def __len__(self):
        return len(self.triples)

    def __getitem__(self, idx):
        pos_triple = self.triples[idx]
        h, r, t = pos_triple
        if self.num_cand == 'all':
            tail_label, head_label = self.get_label(self.hr2t[(h, r)], self.rt2h[(r, t)])
            pos_triple = torch.LongTensor(pos_triple)

            return pos_triple, tail_label, head_label
        else:
            neg_tail_cand = np.random.choice(np.delete(np.arange(self.num_ent), self.hr2t[(h, r)]),
                                             self.num_cand)

            neg_head_cand = np.random.choice(np.delete(np.arange(self.num_ent), self.rt2h[(r, t)]),
                                             self.num_cand)
            tail_cand = torch.from_numpy(np.concatenate(([t], neg_tail_cand)))
            head_cand = torch.from_numpy(np.concatenate(([h], neg_head_cand)))

            pos_triple = torch.LongTensor(pos_triple)

            return pos_triple, tail_cand, head_cand

    def get_label(self, true_tail, true_head):
        y_tail = np.zeros([self.num_ent], dtype=np.float32)
        for e in true_tail:
            y_tail[e] = 1.0
        y_head = np.zeros([self.num_ent], dtype=np.float32)
        for e in true_head:
            y_head[e] = 1.0

        return torch.FloatTensor(y_tail), torch.FloatTensor(y_head)

    @staticmethod
    def collate_fn(data):
        pos_triple = torch.stack([_[0] for _ in data], dim=0)
        tail_label_or_cand = torch.stack([_[1] for _ in data], dim=0)
        head_label_or_cand = torch.stack([_[2] for _ in data], dim=0)
        return pos_triple, tail_label_or_cand, head_label_or_cand
